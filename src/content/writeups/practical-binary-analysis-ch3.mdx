...
<Callout title="Where did .idata go?" variant="amber">
  My binary has no `.idata` or `.edata` section, and neither does the book's example. Linkers routinely merge them into `.rdata`, and the DataDirectory entries still point at the right RVAs, so the loader neither knows nor cares. This is exactly why the DataDirectory exists: the *contents* are found by directory index, not by section name. Never go looking for imports by section name; read entry 1.
</Callout>
...
<Callout title="In my build the thunk is the minority case" variant="amber">
  Counting how each of the 76 IAT slots is actually reached: **62** are reached by a direct `call QWORD PTR [rip+…]` at the call site, and only **16** through a `jmp` thunk (2 slots both ways, for 76 distinct slots in total, exactly the import count, which is a satisfying cross-check). So the book's "a call to a library function is implemented as a call to a thunk" describes the *older* pattern; MSVC now usually emits `call [__imp_Foo]` straight through the IAT and skips the middleman. The thunk still appears, mostly where a function's address is taken rather than immediately called. The mental model survives intact, since the IAT slot is the indirection point either way, but if you go hunting for a PLT-shaped stub per import you will not find one.
</Callout>
