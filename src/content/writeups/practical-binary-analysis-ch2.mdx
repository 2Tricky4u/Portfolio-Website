...
<Callout title="Section names are attacker-controllable, so don't trust them" variant="red">
...
<Callout title="The entry point is _start, and it never returns to itself" variant="teal">
  `e_entry` is `0x1060` = `_start`, injected by the CRT (`crt1.o`), not written by you. It loads the address of `main` into `rdi` (first integer argument) and hands it to `__libc_start_main`, which sets up `atexit`, runs constructors, then calls `main`. When `main` returns, libc calls `exit`, so the `hlt` right after the `call` in `_start` is dead code, a backstop that should never execute.
</Callout>
...
<Callout title="One file, two organizations that don't nest cleanly" variant="blue">
