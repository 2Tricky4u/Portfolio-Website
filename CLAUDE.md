...
- **The contact email is deliberately not a link.** `Contact.astro` spells it out as plain text (`xaga dot ogay at gmail dot com`, symbol words in italic `.sym` spans) and the terminal's `open email` target is assembled from string parts at runtime, so the literal address never appears in the HTML or JS bundle. Don't "fix" it into a `mailto:` or a plain `@` address.
